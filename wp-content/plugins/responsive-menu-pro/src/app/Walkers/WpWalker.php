...
class WpWalker extends \Walker_Nav_Menu {
    ...
    public function __construct(OptionsCollection $options) {
      ...
      if(isset($responsive_menu_classes)):
        if($depth + 1 == $this->options['menu_depth']->getValue() && ($key = array_search('responsive-menu-pro-item-has-children', $responsive_menu_classes)) !== false) {
      ...
      if($this->options['menu_font_icons']):
        ...
        if(is_object($font_icons)):
          ...
          $font_icon = isset($font_icons[$item->ID]) ? "<i class='fa fa-{$font_icons[$item->ID]}'></i>" : "";
        else:
          $font_icon = '';
        endif;
      else:
    ...
    public function start_lvl(&$output, $depth = 0, $args = array()) {
      ...
      if($this->options['use_slide_effect'] == 'on')
        $output .= '<div class="responsive-menu-pro-back">' . $this->options->getActiveArrow() . ' Back to ' . ($this->getParentItem()->title ? $this->getParentItem()->title : 'Main Menu') . '</div>';

      $this->setParentItem($this->getCurrentItem());
